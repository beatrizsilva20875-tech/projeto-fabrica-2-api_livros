...
# make_response = transforma os json em métodos HTTP e permite que estilizemos nossa resposta 
# e trata os erros
...
def get_livro(id):
    for livro in livros: # percorrer a lista de livros
        if livro.get('id') == id:
            return make_response(jsonify(
                mensagem=f'Lidro de ID {id} encontrado.',
                dados = livro
...
def update_livro(id):
    for livro in livros:
        if livro.get('id') == id:
            ...
            livro.update(novo_dados) # substitui todos oos dados
            return make_response(jsonify(
...
if __name__ == '__main__': # esse comando permite que a api seja executada 
    # de maneira independente em outros arquivos
    app.run(debug=True)